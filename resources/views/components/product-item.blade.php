@php
$index = ($product->sale == null || $product->sale->price_sale == $product->price) ? 0 : 1;
@endphp
...
<div class="m-1 border text-center position-relative">
    @if ($index == 1)
        <div class="product-sale ">
            <div class="sale-off">
                -{{ (int) ((($product->price - $product->sale->price_sale) / $product->price) * 100) }}%
            </div>
    ...
    <div class="box-hover">
        <a id="wishlist_product-url{{ $product->id }}" href="{{ route('slug.index', ['slug' => $product->slug]) }}">
            <img class="img-change  img-product img-fluid"
                src="{{ asset('images/product/' . $product->images[0]->image) }} "
                id="wishlist_product-image{{ $product->id }}" alt="{{ $product->images[0]->image }}" />
    ...
    <div class="d-flex   justify-content-around ">
        <div class="icon heart btn_wishlist" id="{{ $product->id }}" onclick="add_wishlist(this.id)">
            <svg xmlns="http://www.w3.org/2000/svg" fill="white" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="w-6 h-6 heart-icon{{ $product->id }}">
        ...
        @if ($index == 0)
            <div>
                <input type="hidden" id="wishlist_product-price{{ $product->id }}"
                    value="{{ number_format($product->price) }}đ">
        ...
        @else
            <div>
                <input type="hidden" id="wishlist_product-price{{ $product->id }}"
                    value="{{ number_format($product->sale->price_sale) }}đ">
        ...
        @if ($product->qty > 0)
            <div class="icon cart" onclick="add2cart(this.id)" id="{{ $product->id }}">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-6 h-6 fa-beat">
                    <path stroke-linecap="round" stroke-linejoin="round"
        ...
        @else
            <div class="icon cart">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-6 h-6 " style="color: #cccc; ">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
                </svg>
